...
func _run() -> void:
	...
	if failures.is_empty():
		print("M5 regression runner passed. Versus pairs: %d, training cases: %d." % [versus_pair_count, training_case_count])
		quit(0)
		return

	for failure in failures:
		push_error(failure)
	print("M5 regression runner failed with %d issues." % failures.size())
	quit(1)
...
func _validate_database_entries() -> void:
	for character_entry in CharacterDatabase.list_characters():
		...
		if String(character_entry.get("display_name", "")).strip_edges() == "":
			failures.append("%s: missing display_name" % character_entry.get("id", "unknown_character"))
		if String(character_entry.get("movement_summary", "")).strip_edges() == "":
			failures.append("%s: missing movement_summary" % character_entry.get("id", "unknown_character"))
		if String(character_entry.get("dash_profile_label", "")).strip_edges() == "":
			failures.append("%s: missing dash_profile_label" % character_entry.get("id", "unknown_character"))
	...
	for weapon_entry in WeaponDatabase.list_weapons():
		...
		if String(weapon_entry.get("display_name", "")).strip_edges() == "":
			failures.append("%s: missing display_name" % weapon_entry.get("id", "unknown_weapon"))
		if String(weapon_entry.get("preview_summary", "")).strip_edges() == "":
			failures.append("%s: missing preview_summary" % weapon_entry.get("id", "unknown_weapon"))
		if String(weapon_entry.get("attack_tempo_label", "")).strip_edges() == "":
			failures.append("%s: missing attack_tempo_label" % weapon_entry.get("id", "unknown_weapon"))
		if String(weapon_entry.get("range_class", "")).strip_edges() == "":
			failures.append("%s: missing range_class" % weapon_entry.get("id", "unknown_weapon"))
...
func _validate_runtime_assembly() -> void:
	for character_entry in CharacterDatabase.list_characters():
		for weapon_entry in WeaponDatabase.list_weapons():
			...
			if String(runtime_data.get("id", "")).strip_edges() == "":
			...
			if String(runtime_data.get("movement_summary", "")).strip_edges() == "":
				failures.append("Runtime assembly missing movement_summary for %s + %s" % [character_entry["id"], weapon_entry["id"]])
			if String(runtime_data.get("weapon_preview_summary", "")).strip_edges() == "":
				failures.append("Runtime assembly missing weapon_preview_summary for %s + %s" % [character_entry["id"], weapon_entry["id"]])
...
func _validate_scene_boot(mode: String, p1_loadout: Dictionary, p2_loadout: Dictionary) -> void:
	...
	if scene.fighters.size() != 2:
		...
		for fighter in scene.fighters:
			...
			if String(fighter.fighter_data.get("movement_summary", "")).strip_edges() == "":
				failures.append("%s missing movement_summary in fighter_data" % label)
			if String(fighter.fighter_data.get("weapon_preview_summary", "")).strip_edges() == "":
				failures.append("%s missing weapon_preview_summary in fighter_data" % label)
